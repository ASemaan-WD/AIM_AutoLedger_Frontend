...
import { getClientId } from './auth-service';
...
export async function uploadFile(file: File, signal?: AbortSignal): Promise<UploadResponse> {
  try {
    ...
    console.log('🔍 Step 2: Checking for duplicate files...');
    ...
    if (duplicateCheck.isDuplicate && duplicateCheck.duplicateRecord) {
      console.warn('⚠️  Duplicate file detected:', duplicateCheck.duplicateRecord.name);
      return {
        success: false,
    ...
    console.log('✅ Uploaded to Vercel:', blobResult.url);

    // Step 4: Create File record in Airtable
    console.log('📝 Step 4: Creating File record in Airtable...');
    ...
    try {
      airtableResponse = await createRecords('Files', {
        fields: {
          ...
          ...(clientId && { 'ClientId': clientId }), // Include ClientId if available
      ...
      console.log('✅ Created Airtable record:', createdRecord.id);
      
    } catch (airtableError) {
      // If Airtable creation fails, we have an orphaned blob
      console.error('❌ Airtable record creation failed');
      console.warn(
        '⚠️  WARNING: Orphaned blob created at:', blobResult.url,
